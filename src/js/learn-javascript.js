...
// 14.1.1 Обычно при чтении несуществующего свойства из объекта возвращается undefined.
...
// 14.1.2 В некоторых языках программирования возможно получать элементы массива, используя отрицательные индексы, отсчитываемые с конца.
// Вот так:
// let array = [1, 2, 3];
...
// 14.1.3 Создайте функцию makeObservable(target), которая делает объект «наблюдаемым», возвращая прокси.
// Вот как это должно работать:
...
// 14.2.1 Создайте калькулятор, который запрашивает ввод какого-нибудь арифметического выражения и возвращает результат его вычисления.
...
// 14.4.1 Каким будет результат выполнения этого кода?
...
// 14.4.2 В представленном ниже коде мы намерены вызвать obj.go() метод 4 раза подряд.
...
// console.log(access);                // 24
// console.log(typeof access);         // number
// let access2 = access.toString(2);   
// console.log(access2);               // 11000
// console.log(typeof access2);        // string

// 14.5.1 Почему побитовые операции в примерах ниже не меняют число? Что они делают внутри?
// console.log(123^0);       // 123
// console.log(0^123);       // 123
// console.log(~~123);       // 123

// 14.5.2 Напишите функцию isInteger(num), которая возвращает true, если num – целое число, иначе false.
...
// 14.5.3 Верно ли, что для любых a и b выполняются равенства ниже?
...
// 14.5.4 Почему результат второго alert такой странный?
...
// 14.8.1 Используя Intl.Collator, отсортируйте массив:
